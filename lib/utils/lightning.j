...
*/
    ...
    struct Lightning extends array
			...
			loop
				...
				if this.dur > 0 or this.dur == -1 then
					...
					if this.source != null and this.target != null then
						call MoveLightningEx(this.l, true, GetUnitX(this.source), GetUnitY(this.source), GetUnitZ(this.source) + Z_OFFSET, GetUnitX(this.target), GetUnitY(this.target), GetUnitZ(this.target) + Z_OFFSET)
					endif
					if this.color then
		...
		static method createUnits takes string codeName, unit source, unit target returns thistype
			...
			set this.l = AddLightningEx(codeName, true, GetUnitX(source), GetUnitY(source), GetUnitZ(source) + Z_OFFSET, GetUnitX(target), GetUnitY(target), GetUnitZ(target) + Z_OFFSET)
			set this.dur = -1
			call this.push()
			return this
		endmethod
