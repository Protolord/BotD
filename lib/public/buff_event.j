...
*/
    struct BuffEvent
        ...
        method destroy takes nothing returns nothing
            ...
            call RemoveSavedInteger(Buff.priv_hash, GetHandleId(this.cf), 0)
            set this.tc = null
            set this.cf = null
        ...
        static method remove takes code c returns nothing
            ...
            if HaveSavedInteger(Buff.priv_hash, id, 0) then
                call thistype(LoadInteger(Buff.priv_hash, id, 0)).destroy()
            debug else
        ...
        static method pickAll takes nothing returns nothing
            static if LIBRARY_TimerUtils then
                ...
                set thistype.buff = LoadInteger(Buff.priv_hash, id, 0)
                call RemoveSavedInteger(Buff.priv_hash, id, 0)
        ...
        static method create takes code c returns thistype
            ...
            local thistype this
            if HaveSavedInteger(Buff.priv_hash, id, 0) then
                set this = thistype(LoadInteger(Buff.priv_hash, id, 0))
            else
                set this = thistype.allocate()
                set this.tc = TriggerAddCondition(thistype.trg, cf)
                set this.cf = cf
                call SaveInteger(Buff.priv_hash, id, 0, this)
            endif
            return this
        endmethod
    endstruct
