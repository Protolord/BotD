        ...
        private static method onDamage takes nothing returns nothing
            ...
            if Damage.type == DAMAGE_TYPE_PHYSICAL and not Damage.coded and thistype.tb.has(id) then
                if TargetFilter(Damage.target, GetOwningPlayer(Damage.source)) then
                    ...
                    call Damage.element.apply(Damage.source, Damage.target, amount, ATTACK_TYPE, DAMAGE_TYPE, DAMAGE_ELEMENT_SPIRIT)
                    call Heal.unit(Damage.source, Damage.source, amount, 1.0, true)
                endif
